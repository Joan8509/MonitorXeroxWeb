...
def _b7135_fuser_status(host: str):
    ...
    try:
        ...
        driver.get(url)
        time.sleep(6)  # esperar carga de página
...
# --------- B7135: Lectura Fuser/Transfer R7 vía interfaz web (con caché) ---------
...
def _b7135_web_status(host: str) -> Dict[str, str]:
    ...
    Mantiene un caché de 1 hora para evitar abrir el navegador en cada consulta.
    """
    import time
    from selenium import webdriver
    from selenium.webdriver.chrome.options import Options
    ...
    try:
        ...
        # 🔹 Espera dinámica: hasta que aparezca texto "fuser" o "transfer roller"
        import time
        start = time.time()
        page = ""
        while time.time() - start < 12:  # máximo 12 s
...
def fetch_supplies_generic(ip: str, community: str = DEFAULT_COMMUNITY, timeout: int = TIMEOUT_DEFAULT) -> Dict[str, Any]:
    ...
    if model == "B7135":
        try:
            # Prioridad: leer desde el panel web (caché 1h) para Fuser R8 y Transfer Roller R7
        ...
        # Si alguno sigue Unknown, intentar híbrido por SNMP (alertas + OID privado)
        if fuser_status == "Unknown" or tr_status == "Unknown":
            try:
                hybrid = _b7135_r7_r8_status(ip, community, timeout)
            except Exception as e:
                print(f"[B7135] Error híbrido R7/R8: {e}")
                hybrid = {}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* Ajuste específico del textarea de IPs */
  ...
  <div style="position:relative;z-index:9999;display:grid;place-items:center;min-height:100%;">
    <div style="width:min(92vw,560px);background:#fff;border:1px solid #e5e7eb;border-radius:18px;box-shadow:0 25px 80px rgba(2,6,23,.35);padding:18px 18px 14px;">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
        ...
        <button id="pmCloseX" type="button" aria-label="Close" style="border:0;background:#f1f5f9;border-radius:12px;width:36px;height:36px;cursor:pointer;font-size:18px;font-weight:900;color:#0f172a;">×</button>
      </div>
  ...
  <script>
    ...
    function pivotRowHTML(d){
      ...
      return `<tr>
        ...
        <td><a href="#" class="printer-link" data-ip="${d.ip||''}">${(d.printer_name||'').trim()||'(unnamed)'}</a></td>
...
def api_supplies_list():
    """
    Consulta múltiple optimizada:
    - B7135 → modo concurrente limitado (máx 4 ChromeDriver)
    - Otros modelos → threading normal (rápido)
    """
    import threading
    ...
    # Detectar modelos primero (solo SNMP, muy rápido)
    ...
    # --- Cola controlada para B7135 (máx 4 en paralelo) ---
    q = Queue()
    for ip in b7135_ips:
        q.put(ip)
    ...
    # Lanzar 4 hilos Selenium simultáneos
    max_parallel = 4
